...
const CustomDashboard: React.FC = () => {
    ...
    const modalNoteRef = useRef<DashboardNoteHandle | null>(null);
    ...
    const handleCloseModal = async () => {
        if (modalNoteRef.current) {
            try {
                await modalNoteRef.current.save();
            } catch (err) {
    ...
    const fetchDashboard = async (isInitial = false) => {
        try {
            ...
            const res = await axios.get('/dashboard', { params: { uploadId: currentUploadId } });
    ...
    const saveDashboard = useCallback(async (currentItems: DashboardItem[], showToast = false) => {
        try {
            setSaving(true);
            await axios.post('/dashboard/save', { items: currentItems, uploadId: currentUploadId });
            if (showToast) {
    ...
    const debouncedSave = useCallback(
        debounce((currentItems: DashboardItem[]) => {
            return saveDashboard(currentItems);
        }, 2000),
        [saveDashboard]
    );
    ...
    const addItem = async (type: 'text' | 'title') => {
        try {
            ...
            const newItem = {
                type,
                title: isTitle ? 'New Title' : 'New Note',
                content,
                layout: {
                    x: 0,
                    y: Infinity,
            ...
            setEditingItem(savedItem); // Open the editor immediately for the new item
        } catch (err) {
    ...
    const removeItem = async (id: string) => {
        try {
            await axios.delete(`/dashboard/item/${id}`, { params: { uploadId: currentUploadId } });
    ...
    const handleTitleUpdate = (id: string, newTitle: string) => {
        ...
        setItems(prev => {
            const newItems = prev.map(item => item.id === id ? { ...item, title: newTitle } : item);
            debouncedSave(newItems);
            return newItems;
        });
        setEditingTitleId(null);
    ...
    const handleAnalyzeGraph = async (item: DashboardItem) => {
        ...
        try {
            ...
            const response = await axios.post('/analyze-graph', analysisPayload);

            if (response.data.status !== 'success' || !response.data.analysis) {
                throw new Error(response.data.message || 'Analysis failed to return result');
            }
        ...
        <div className="custom-dashboard-page">
            ...
            <div className="unified-page-header">
                ...
                <div className="header-actions">
                    <button onClick={() => addItem('title')} className="btn-action">
                        <FileText size={16} /> Add Title
                    </button>
                ...
                <ErrorBoundary>
                    {items.length > 0 && (
                        <div className="dashboard-grid-wrapper" style={{ position: 'relative' }}>
                            ...
                            {(() => {
                                const rowHeight = 2;
                                const PAGE_HEIGHT = 1325; // Adjusted for new A4 zoom scale to prevent bleeding
                                            ...
                                            <div className="item-header drag-handle">
                                                {editingTitleId === item.id ? (
                                                    <input
                                                        ...
                                                        onBlur={(e) => handleTitleUpdate(item.id, e.target.value)}
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter') handleTitleUpdate(item.id, e.currentTarget.value);
                                                            else if (e.key === 'Escape') setEditingTitleId(null);
                                                        }}
                                                        onMouseDown={(e) => e.stopPropagation()} // Prevent dragging while clicking input
                                                    />
                                                ) : (
                                                    <span
                                                        className="item-title"
                                                        onClick={(e) => {
                                                            e.stopPropagation(); // Preven trigger of other events
            ...
            {/* Edit Modal for Notes/Titles */}
            {editingItem && (
                <div className="modal-overlay" onClick={handleCloseModal}>
                    <div className="modal-content text-editor-modal" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3>Edit {editingItem.type === 'title' ? 'Title' : 'Note'}</h3>
                            <button className="btn-close-modal" onClick={handleCloseModal}>
                                <X size={20} />
                            </button>
                        </div>
                        <div className="modal-body editor-container">
                            <DashboardNote
                                noteId={editingItem.content.noteId}
                                initialText={editingItem.content.text}
                                isTitle={editingItem.type === 'title'}
                                ref={modalNoteRef}
                                onChange={(newText) => {
                                    setItems(prev => prev.map(item =>
                                        item.id === editingItem.id
                                            ? { ...item, content: { ...item.content, text: newText } }
                                            : item
                                    ));
                                }}
                            />
                        ...
                        <div className="modal-footer">
                            <p className="modal-hint">Changes are saved automatically as you type.</p>
                            <button className="btn-confirm-delete" style={{ background: 'var(--accent-secondary)', color: 'white', border: 'none' }} onClick={handleCloseModal}>
                                Done
                            </button>
                        </div>
                    </div>
